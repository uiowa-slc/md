...
<section class="portfolio-post">
    ...
    <div class="portfolio-post__image-list main-image row row--large">
        <div class="large-12">
            <% if $Image %><img class="dp-lazy $FirstLast" src="{$ThemeDir}/dist/images/placeholder.png" data-original="$Image.SetWidth(1400).URL" data-original-small="$Image.SetWidth(420).URL" data-original-medium="$Image.SetWidth(768).URL" alt="Cover photograph for $Title"><% end_if %>
             <noscript><img src="$Image.SetWidth(600).URL" alt="" role="presentation" /></noscript>
        </div>
    </div>
    <div class="portfolio-post__heading active row">
        <div class="columns large-12 portfolio-post__summary">
            <% if $SiteLink %><h1><a target="_blank" href="$SiteLink">$Title  <i class="fa fa-external-link" aria-hidden="true"></i></a></h1>
